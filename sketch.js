...
new p5((p) => {
    ...
    p.draw = () => {
        if (settings.followMouse && !isGenerating) {
            ...
            // For mouse interaction to feel good, we should probably 
            // re-generate the curves or at least re-calculate their paths
            // But even just re-rendering existing ones with new field influence 
            // requires re-calculating points.

            // To keep it simple and performant, let's just re-render 
            // if we are in a mode that supports it.
            // Actually, for true interactive flow, we should re-create curves.
            curveManager.init(); // Heavy but correct for real-time sculpting
            curveManager.render();
        }
